...
class TestProjectsEndpoints(unittest.TestCase):
    ...
    def test_create_connection_from_urls(self):
        ...
        res = self.client().post(f'/projects/{self.project_1.id}/connections', json=json_body, headers=auth_header)
        data = json.loads(res.data)
    ...
    def create_connection_from_nonexistent_source_from_urls(self):
        ...
        source = Source.query.filter(Source.project_id == self.project_1.id, Source.url == self.new_source_url).first()
    ...
    def test_create_connection_no_body_from_urls(self):
        res = self.client().post(f'/projects/{self.project_1.id}/connections', headers=auth_header)
        data = json.loads(res.data)
